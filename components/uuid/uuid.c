...
void initsettingsuuid()
{
    FILE *f = fopen("/spiffs/uuid.txt", "r");
    if (f == NULL) {
        ESP_LOGE("uuid", "Failed to open file for reading");
        uuid_t uu;
        uuid_generate(uu);
        // ESP_LOG_BUFFER_HEXDUMP(TAG, uu, sizeof(uuid_t), ESP_LOG_WARN);
        char uu_str[UUID_STR_LEN];
        uuid_unparse(uu, uu_str);
        ESP_LOGW("uuid", "parsed: %s", uu_str);
        FILE *f = fopen("/spiffs/uuid.txt", "w");
        ESP_LOGI("uuid", "fopen(/spiffs/uuid.txt");
        fprintf(f, uu_str);
        fclose(f);
    } else {
        char my_json_string2[UUID_STR_LEN];
        fgets(my_json_string2, sizeof(my_json_string2), f);
        ESP_LOGW("uuid", "parsed: %s", my_json_string2);
        fclose(f);
        strncpy(uuidcid, my_json_string2, UUID_STR_LEN);
    }
}